...
export const HoverSlider = React.forwardRef<
  HTMLElement,
  React.HTMLAttributes<HTMLElement> & { autoSlideInterval?: number; slidesCount?: number }
>(({ children, className, autoSlideInterval, slidesCount, ...props }, ref) => {
  ...
  React.useEffect(() => {
    if (!autoSlideInterval || !slidesCount) return;
    const interval = setInterval(() => {
      setActiveSlide((prev) => (prev + 1) % slidesCount);
    }, autoSlideInterval);
    return () => clearInterval(interval);
  }, [autoSlideInterval, slidesCount]);
